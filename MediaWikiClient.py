class MediaWikiClient:
    ...
    def getUserInfo(self):
        ...
        result = self.apiRequest({'action':'query', 'meta':'userinfo', 'uiprop':self.listToString(properties)}) #fix this when the query method is implemented
    ...
    def query(self, titles = [], pageIds = [], revIds = [], list = [], meta = [], generator = '', redirects = True, convertTitles = False, indexPageIds = False, export = False, exportNoWrap = False, iwUrl = False, extraParams = {}):
        values = {'action':'query', 'titles':self.listToString(titles), 'pageids':self.listToString(pageIds), 'revids':self.listToString(revIds), 'list':self.listToString(list), 'meta':self.listToString(meta), 'generator':generator}
